...
auto vhook::trampoline_hook( uintptr_t function , uintptr_t custom_function , size_t instance_size ) -> uintptr_t
{
    ...
    auto clone = reinterpret_cast< uintptr_t >( VirtualAlloc( nullptr , instance_size + extra_size , MEM_COMMIT | MEM_RESERVE , PAGE_EXECUTE_READWRITE ) );
    if ( !clone ) return 0;

    memmove( reinterpret_cast< void* >( clone ) , reinterpret_cast< void* >( function ) , instance_size );
    ...
    *( uintptr_t* ) ( clone + instance_size + 1 ) = ( function - clone - extra_size );
    ...
    if ( !VirtualProtect( reinterpret_cast< void* >( function ) , instance_size , PAGE_EXECUTE_READWRITE , &old_protect ) )
    {
        printf( "Could not change function's memory page protection." );
        VirtualFree( reinterpret_cast< void* >( clone ) , 0 , MEM_RELEASE );
        return 0;
    }

    memset( reinterpret_cast< void* >( function ) , 0x90 , instance_size );

    *( uint8_t* ) ( function ) = 0xE9;
    *( uintptr_t* ) ( function + 1 ) = ( custom_function - function - extra_size );

    if ( !VirtualProtect( reinterpret_cast< void* >( function ) , instance_size , old_protect , &old_protect ) )
    {
...
auto vhook::get_module_base( ) -> uintptr_t
{
	...
	const ntos::ldr_data_table_entry* main_mod = reinterpret_cast< ntos::ldr_data_table_entry* >( contains_record( main_mod_entry , ntos::ldr_data_table_entry , InMemoryOrderLinks ) );
...
auto vhook::get_section_by_name( const char* sectionName ) -> SectionData
{
	...
	IMAGE_DOS_HEADER* dos_headers = reinterpret_cast< IMAGE_DOS_HEADER* >( mod );
	IMAGE_NT_HEADERS* nt_headers = reinterpret_cast< IMAGE_NT_HEADERS* >( reinterpret_cast< BYTE* >( mod ) + dos_headers->e_lfanew );
	...
	{
		const char* curr_section_name = reinterpret_cast< const char* >( section_header->Name );
		if ( strcmp( curr_section_name , sectionName ) == 0 )
		{
			uintptr_t sectionStart = reinterpret_cast< uintptr_t >( mod ) + section_header->VirtualAddress;
...
auto vhook::iat_hook( const char* function_name , orig_test_func custom_func ) -> BOOL
{
	...
	while ( import_descriptor->Name )
	{
		...
		while ( thunk_data->u1.Function )
		{
			...
			if ( strcmp( reinterpret_cast< char* >( import_by_dawg->Name ) , function_name ) == 0 )
			{
				DWORD oldProtect;
